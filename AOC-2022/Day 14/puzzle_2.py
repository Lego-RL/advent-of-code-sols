...
def can_sand_move(
    rock_lines: set[Point], sand_points: set[Point], sand: Point, floor_level: int
) -> Point | None:
    """
    ...
    # can't move anywhere if floor directly below
    if sand.y + 1 == floor_level:
        return None
...
def drop_sand(rock_lines, sand_points: set[Point], floor_level: int):
    """
    ...
    while next_point := can_sand_move(rock_lines, sand_points, sand, floor_level):

        # don't go down any further if floor level is sand
        if next_point.y == floor_level:
            return sand
...
rock_lines: set[Point] = set()
sand_points: set[Point] = set()
deepest_y: int = 0
...
rock_lines = set(rock_lines)
floor_level: int = deepest_y + 2
num_sand_at_rest: int = 0
while new_sand := drop_sand(rock_lines, sand_points, floor_level):
    sand_points.add(new_sand)
    num_sand_at_rest += 1

    if new_sand == Point(500, 0):
        break
